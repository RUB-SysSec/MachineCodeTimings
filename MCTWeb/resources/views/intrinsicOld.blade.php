...
@section('content')
    ...
    @if (isset($intrinsic_prev))
        <li class="pull-left"><a href="/intrinsics/list/@if ($type == 'saved')saved/@endif{{ $intrinsic_prev->id }}"><span class="glyphicon glyphicon glyphicon-chevron-left" aria-hidden="true"></span>{{ $intrinsic_prev->intrinsic }}</a></li>
    @endif
    
    @if (isset($intrinsic_next))
        <li class="pull-right"><a href="/intrinsics/list/@if ($type == 'saved')saved/@endif{{ $intrinsic_next->id }}">{{ $intrinsic_next->intrinsic }}<span class="glyphicon glyphicon glyphicon-chevron-right" aria-hidden="true"></span></a></li>
    @endif
                        ...
                        <thead> 
                            <tr>
                                ...
                                @foreach($intrinsic->parameters as $parameter)
                                    <th>[Type: {{$parameter->type->id}}] [Id: {{$parameter->id}}] {{$parameter->type->type}} {{$parameter->name}}</th> 
                                @endforeach
                            </tr> 
                        </thead> 
    ...
    <div class="panel panel-default">
        ...
        <div class="panel panel-success" style="margin:5px;">
            <div class="panel-heading">
                <h3 class="panel-title"><a href="/nodes/list/{{$node_id}}">{{$node->name}}</a> [<a href="/highcharts/instruction/{{$id}}/job/{{$job->id}}/node/{{$node_id}}">JSON</a>]</h3>
            </div>        
            <div class="panel-body" >
                <div id="highchart_{{$job->id}}{{$node_id}}" data-loaded="false"></div>
            </div>
        </div>
        @endif
        @php
